...
export const liquidballComponent: ComponentData = {
  name: "LiquidBall",
  description:
    "An animated liquid ball component with realistic water physics and customizable percentage display.",
  preview: () => import("@/src/registry/default/ui/LiquidBall"),
  demo: `"use client";
import LiquidBall from "@/src/components/ui/LiquidBall";

export function LiquidBallDemo() {
  return (
    <div className="flex items-center justify-center min-h-[400px] gap-8">
      <LiquidBall initialPercentage={25} size={80} />
      <LiquidBall initialPercentage={50} size={100} />
      <LiquidBall initialPercentage={75} size={120} />
      <LiquidBall initialPercentage={90} size={140} />
    </div>
  );
}`,
  code: `"use client"
...
export default function LiquidBall({ initialPercentage = 75, size = 80 }) {
  ...
  cli: "npx shadcn@latest add https://qi.kanakk.me/r/liquidball.json",
  manualSteps: [
    {
...
export default function LiquidBall({ initialPercentage = 75, size = 40 }) {
  const [percentage, setPercentage] = useState(initialPercentage)
  const canvasRef = useRef<HTMLCanvasElement | null>(null)
  const containerRef = useRef<HTMLDivElement>(null)
  const [dimensions, setDimensions] = useState({ width: size, height: size })
  ...
  return (
    ...
    >
      ...
      language: "tsx",
      filename: "components/ui/LiquidBall.tsx",
    },
  ],
}; 